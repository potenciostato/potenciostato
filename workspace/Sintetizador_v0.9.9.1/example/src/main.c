 ...
 *
 */
// 0.9.7.8 Se integra USB al proyecto casi en su totalidad
//TODO: hacer andar las teclas del teclado en QT
//TODO: si cambio en el sinte tiene que cambiar en la app
//TODO: poder guardar la configuracion desde la pc y desde el teclado
...
xSemaphoreHandle SONIDO;
xSemaphoreHandle LECTURA;
xSemaphoreHandle DISPLAY;
xSemaphoreHandle GENONDA;
xSemaphoreHandle ATTACK;
xSemaphoreHandle COLGADO;
...
xTaskHandle GenOnda, Controles, Display, Botones, Encoders, Teclas;
/*****************************************************************************
 * Declaracion de funciones
 ****************************************************************************/

int Leer_Encoder(int); void Tomar_Encoders(void);

void Leer_Teclado(void); void Prender_Leds(void); void Leer_Botones(void);

char * itoa_mio(int entero); char * itoa_mio_porcentual(int entero);

void hostDispositivo(int id, int valor);
void hostDispositivoTeclado(int id, int valor1, int valor2);
...
static void vLeerBotones(void *pvParameters) {
	...
	static int menu_teclado = 0, opcion_elegida = 0;
	...
	while(1){
		...
		if (!Chip_GPIO_GetPinState(LPC_GPIO, PORTBOTONES, PINSENO)==1){
			...
			if (uxQueueMessagesWaiting(cola_formaDeOnda)==1){
				xQueueReceive( cola_formaDeOnda, &formaDeOndaaux, (portTickType) 0 );
			}
			xQueueSendToBack( cola_formaDeOnda, &formaDeOnda, (portTickType) 0);
		}
		...
		if (!Chip_GPIO_GetPinState(LPC_GPIO, PORT_BOT_VOL, PIN_BOT_VOL)==1){
			...
			if (uxQueueMessagesWaiting(cola_menu_teclado)==1){
				xQueueReceive( cola_menu_teclado, &menu_teclado, (portTickType) 0 );
			}
			if (menu_teclado == 0)
			{
				menu_teclado = 1;
				xQueueSendToBack(cola_menu_teclado, &menu_teclado, (portTickType) 10);
			}
			if (menu_teclado == 1){
				if (uxQueueMessagesWaiting(cola_opcion_elegida)==1){
					xQueueReceive( cola_opcion_elegida, &opcion_elegida, (portTickType) 0 );
				}
				if (opcion_elegida == 0){
					menu_teclado = 0;
					xQueueSendToBack(cola_menu_teclado, &menu_teclado, (portTickType) 10);
...
static void vControles(void *pvParameters) {
	...
	while(1){
		...
		for (i = 0; i < 6; i++){
			PHASE_A = Chip_GPIO_GetPinState(LPC_GPIO, PORTS_FASEA(i), PINS_FASEA(i)); //el 2 no andaa
			PHASE_B = Chip_GPIO_GetPinState(LPC_GPIO, PORTS_FASEB(i), PINS_FASEB(i)); //revisar conexion
			...
			if( diff & 1 ){               // bit 0 = valor (1)
				...
				//enc_delta += (diff & 2) - 1;        // bit 1 = direction (+/-) -- 1 = gire para un lado, 0 no gire, -1 = gire para el otro lado
				enc_delta += (diff & 2) - 1;
			}
		...
		for(i=0;i<6;i++){
			...
			val = enc_delta;
			enc_delta = val & 3;			//para leer encoders de 4 pasos
//			enc_delta = val & 1;			//para leer encoders de 2 pasos
//			enc_delta = 0;					//para leer encoders de 1 paso

			encoders.delta[i] = enc_delta;
			Leer_Encoder[i] = val >> 2;				//para leer encoders de 4 pasos
//			return val >> 1;				//para leer encoders de 2 pasos
//			return val;						//para leer encoders de 1 paso
		}
...
static void vGenFormaOnda(void *pvParameters) {
	...
	uint32_t estadoTeclado=0, estadoTeclado_PREV=0, FORMADEONDA=CUADRADA, i, j=0, timeoutDMA, FILTRO=0, FILTRO_PREV, SET_FILTRO=0, SET_FORMADEONDA=1, FORMADEONDA_PREV=0, SET_VOLUMEN=0,
			SET_ATTACK, ATTACK, ATTACK_PREV, contATTACK=0, volumen = VOLUMEN_INICIAL, CLOCK_DAC_HZ, FCI=CANT_FCI/2, FCI_PREV, FCS=CANT_FCS/2, FCS_PREV, startup=1, OVER=0, OVER_PREV=0, SET_OVER=0;
	...
	float32_t tabla_buffer_f32 [ NUMERO_MUESTRAS ], tabla_aux_f32 [ NUMERO_MUESTRAS ], PASO_ATT=0;

	float32_t *TABLA_FCI[CANT_FCI], *TABLA_FCS[CANT_FCS],  *TABLA_BP[CANT_BP];

    float32_t AMPLITUD=1, volumenfloat, overfloat;
	...
	Chip_GPDMA_PrepareDescriptor ( LPC_GPDMA , &DMA_LLI_buffer  , (uint32_t) tabla_buffer ,
	...
	while (1) {
		...
		if (uxQueueMessagesWaiting(cola_over)==1) {
			xQueuePeek( cola_over, &OVER, (portTickType) 0 );
			if(OVER == OVER_PREV) SET_OVER = 0;
			else {
				overfloat = OVER/100 + 1;
				SET_OVER = 1;
			}
			OVER_PREV = OVER;
		}
		...
		if (uxQueueMessagesWaiting(cola_att)==1) {
			...
			ATTACK_PREV = ATTACK;
			PASO_ATT = (volumenfloat / ATTACK)/100;
		...
		if(SET_ATTACK == 1){
			...
			if(estadoTeclado!=estadoTeclado_PREV){
				...
				contATTACK++;
				if((contATTACK > 10) && (AMPLITUD < volumenfloat/100)){
					AMPLITUD+= PASO_ATT;
					SET_VOLUMEN = 1;
				}
			}
		...
		if((SET_FORMADEONDA == 1 || SET_VOLUMEN == 1 || SET_FILTRO == 1) && FILTRO == 0){
			...
			switch(FORMADEONDA) {
				...
				case CUADRADA:
					...
					tabla_buffer [i] = ( tabla_cuad [i] >> 6);
					tabla_buffer[i]= AMPLITUD * tabla_buffer[i];
					tabla_buffer [i] = ( tabla_buffer [i] << 6);
					}
		...
		if(SET_OVER == 1){
			for ( i = 0 ; i < NUMERO_MUESTRAS ; i++ ) {
				tabla_buffer[i] = ( tabla_sier [i] >> 6);
				if ((overfloat * tabla_buffer[i]) <= 1023){
					tabla_buffer[i] = overfloat * tabla_buffer[i];
				}
				else tabla_buffer[i] = 1023;
				tabla_buffer[i] = tabla_buffer[i] / overfloat;
				tabla_buffer[i] = ( tabla_buffer [i] << 6);
			}
			SET_OVER = 0;
		}
		...
		while (1) {
			...
			if(SET_DELAY == 0){
				...
				if(estadoTeclado !=0 && j>=0){
					timeoutDMA = CLOCK_DAC_HZ / ( FREC_NOTA[COLA_TECLAS[0]] * NUMERO_MUESTRAS ); // Fija la separacion entre muestras.
					Chip_DAC_SetDMATimeOut(LPC_DAC, timeoutDMA);
					if(j > 0) j--;
				...
				timeoutDMA = CLOCK_DAC_HZ / ( FREC_NOTA[DELAY_BUF[0]] * NUMERO_MUESTRAS ); // Fija la separacion entre muestras.
				Chip_DAC_SetDMATimeOut(LPC_DAC, timeoutDMA);
			...
			if((estadoTeclado == 0 && SET_DELAY == 0) || (DELAY_BUF[0]== SILENCIO && SET_DELAY == 1)) {
				...
				if(DMA_SET == 0){
					estadoDMA = Chip_GPDMA_SGTransfer (LPC_GPDMA , Canal_Libre , &DMA_LLI_buffer , GPDMA_TRANSFERTYPE_M2P_CONTROLLER_DMA);
					DMA_SET=1;
				}
			}
			if(estadoDMA == ERROR){
...
int main(void)
{
	...
	vSemaphoreCreateBinary(SONIDO);
	xSemaphoreTake(SONIDO,(portTickType) 10);
	vSemaphoreCreateBinary(LECTURA);
	xSemaphoreTake(LECTURA,(portTickType) 10);
	vSemaphoreCreateBinary(DISPLAY);
	xSemaphoreTake(DISPLAY,(portTickType) 10);
	vSemaphoreCreateBinary(GENONDA);
	xSemaphoreTake(GENONDA,(portTickType) 10);
	vSemaphoreCreateBinary(ATTACK);
	xSemaphoreTake(ATTACK,(portTickType) 10);
	vSemaphoreCreateBinary(COLGADO);
	xSemaphoreTake(COLGADO,(portTickType) 10);
	...
	xTaskCreate(vControles, (signed char *) "Controles",
				configMINIMAL_STACK_SIZE, NULL, (tskIDLE_PRIORITY + 1UL),
				(xTaskHandle *) Controles);
