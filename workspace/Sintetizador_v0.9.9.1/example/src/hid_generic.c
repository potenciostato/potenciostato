...
static ErrorCode_t HID_Ep_Hdlr(USBD_HANDLE_T hUsb, void *data, uint32_t event)
{
	...
	int a[4]={0,0,0,0};
	int i, id, valor, valor_high, valor_low;
	...
	case USB_EVT_OUT:
		...
		case 11:
		{
			valor_low = a[3];
			valor_high = a[2];
			hostDispositivoTeclado(id, valor_high, valor_low);
		}
		default:{
			//nada
		}
		...
		USBD_API->hw->WriteEP(hUsb, pHidCtrl->epin_adr, loopback_report, 4);
		break;
	}
	return LPC_OK;
}
...
//extern void aplicacion_dispositivo(int);
